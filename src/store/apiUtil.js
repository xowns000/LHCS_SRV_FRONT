...
api.interceptors.request.use(
  async (request) => {
    ...
    if(request.url !== '/api/svy/preview/aesurldecrypt'){
      if( request.head ) {
...
api.interceptors.response.use(
  async (response) => {
    ...
    //TelewebJSON 리턴값이 jsonSuper로 감싸져 있을 시, HEADER, DATA를 상위 레벨로 올린다.
    //이메일 발송 시 jsonSuper로 감싸져 리턴되어 추가함. - 2024.02.21. by hjh.
    if(response.data.jsonSuper != undefined && response.data.jsonSuper != null ) {
      response.data.HEADER = response.data.jsonSuper.HEADER;
      response.data.DATA = response.data.jsonSuper.DATA;
    }

    store.dispatch("commonStore/AC_COMMON_PROGRESS_BAR", false);  //응답시 progress_bar = false
    // header.token 자동 갱신
  ...
  async (error) => {
    console.log( "apiUtil.js/axios.interceptors.response.use :: error :: {}" ,error );
    store.dispatch("commonStore/AC_COMMON_PROGRESS_BAR", false);  //응답시 progress_bar = false
    if( (error.toString()).indexOf("status code 403") > -1) {
      ...
      try{
        if( error.response.status == 401) {
          window.sessionStorage.clear();
          location.href="/login";
        }else if (error.response.status == 500 && (error.response.data.exception.indexOf("security.SignatureException") > -1) ) {
          window.sessionStorage.clear();
          location.href="/login";
        }
      }catch(e){}
