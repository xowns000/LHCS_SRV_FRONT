<template>
  ...
  <div :class="`pl-svy-preview ${vFlag && isUsePreviewCss ? 'is-preview' : 'is-survey'}`" :style="`${vFlag ? 'height: calc(100vh - 63px)' : ''}`">
    ...
    <template v-if="SVY_STEP === 'start'">
      <template v-if="SVY_HEAD_DATA.SRVY_NM">
        <div class="pl-svy-preview-wrap flex-grow-1">
          ...
          <div class="pl-svy-preview-content" >
            ...
            <template
              ...
              <strong>응답기간 : <br/>{{ SVY_HEAD_DATA.SRVY_DT }}</strong><br/>
            </template>
            <strong>설문문항 개수 : {{QITEM_LIST.length}}문항</strong>
          </div>
        </div>
    ...
    <template v-else>
      <div class="pl-svy-preview-wrap" >
        <h1 :class="`site-${SITE}`"><span class="is-ellips">{{ SVY_HEAD_DATA.SRVY_QITEM_GROUP_NM }}</span></h1>
        <div class="pl-svy-preview-content" >
          <!-- 개인정보 수집 동의 -->
          <template v-if="SVY_STEP === 'terms'">
            ...
            <div class="pl-form-inline-wrap vertical is-mt-m current-item">
          ...
          <template v-if="svy_status === 'past'">
            <div class="d-flex flex-grow-1 flex-column align-center justify-center flex-wrap is-mt-l">
              <v-icon class="pl-survey-icon is-done"></v-icon>
              <strong class="text-h6 font-weight-bold is-mt-l text-center">본 설문은 종료되었습니다.</strong>
              <p class="is-mt-s text-center">
                관심을 가져 주셔서 감사합니다.
              </p>
            </div>
          ...
          <template v-if="svy_status === 'psnal'">
            <div class="d-flex flex-grow-1 flex-column align-center justify-center flex-wrap is-mt-l">
              <v-icon class="pl-survey-icon is-done"></v-icon>
              <strong class="text-h6 font-weight-bold is-mt-l text-center">본 설문은 참여 인원을 제한하고 있습니다.</strong>
              <p class="is-mt-s text-center">
                이미 제한된 인원수가 모두 참여를 완료하였기 때문에, 추가적인 설문 참여는 불가능합니다.<br/>
                설문에 참여해주신 모든 분들께 진심으로 감사드립니다.
              </p>
            </div>
          </template>
          <!-- 설문 기간이 아닌 경우 -->
          <template v-if="svy_status === 'period'">
            <div class="d-flex flex-grow-1 flex-column align-center justify-center flex-wrap is-mt-l">
              <v-icon class="pl-survey-icon is-done"></v-icon>
              <strong class="text-h6 font-weight-bold is-mt-l text-center">설문 기간이 아닙니다.</strong>
            </div>
          </template>
        </div>
      </div>
...
import { mapState, mapGetters, mapActions } from "vuex";
  ...
  export default {
    ...
    data() {
      return {
        SITE: process.env.VUE_APP_SITE ? process.env.VUE_APP_SITE : 'DEFAULT',
        CUSTCO_NM: '',
        uid: "",
        upwd: "",
        ENCRYPT_KEY: "",
        ...
        //미리보기 여부
        vFlag: true,
        //미리보기 css 사용 여부 - 컴포넌트로 모바일폰 이미지 안에서 미리보기 시 사용함, 브라우저로 미리보기 시, 사용 안함.
        ...
        MESSAGE : {
          ...
          ALERT : {
            ...
            , REG_QITEM : {alertDialogToggle: true, msg: '설문 문항을 등록해 주세요.', iconClass: 'is-info', type: 'default'}
          },
          ERROR : {
    ...
    async mounted() {
      ...
      if(this.mixin_isEmpty(this.previewCall)){
        ...
        if(this.$route.query.key != undefined){
          ...
          this.PARAM_PREVIEW = this.$route.query.preview;
          if(!this.mixin_isEmpty(this.PARAM_PREVIEW) && this.PARAM_PREVIEW == 'Y') {
            this.vFlag = true;
    ...
    methods: {
      ...
      errChk(resData){
        ...
        else if(resData.HEADER.ERROR_MSG === 'PRD') this.pageErrCall('period');
      ...
      getSrvyGroup(goWhere){
        ...
        if(this.GROUP_LIST.length == 0) {
          this.showAlert(this.MESSAGE.ALERT.REG_QITEM);
          return;
        }
      ...
      async nextSrvyQitem() {
        //이전/다음 실행중이면 return
        if(this.isRunPrevNext) {
          return
        }
        this.isRunPrevNext = true;
