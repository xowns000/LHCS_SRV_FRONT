<template>
  <div class="pl-svy-preview" :style="`height: calc(100vh - 63px)`">
    <!-- 첫화면 -->
    <template v-if="SVY_STEP === 'start'">
      <div class="pl-svy-preview-wrap flex-grow-1">
        <img :src="SVY_HEAD_DATA.IMG_URL" :alt="SVY_HEAD_DATA.SRVY_QITEM_GROUP_NM" >
    ...
    <template v-else>
      <div class="pl-svy-preview-wrap">
        ...
        <div class="pl-svy-preview-content" >
          ...
          <template v-if="SVY_STEP === 'terms'">
            ...
            <div class="pl-desc">한국클라우드는 “개인 정보 보호법”에 따라 동의를 얻어 아래와 같이 서비스의 홍보 및 마케팅을 위한 개인 정보를 수집이용합니다. </div>
          ...
          <!-- 단일 선택형 설문 -->
          <template v-if="SVY_STEP === 'SNGL'">
            <h2 class="pl-subtit">{{ SVY_ITEM_DATA.SRVY_QITEM_GROUP_NM }}</h2>
            <div class="pl-form-inline-wrap vertical type-2 is-mt-s ">
              <div class="pl-form-inline">
                <span class="pl-label">
                  <span 
                    v-if="SVY_ITEM_DATA.ESNTL_YN == 'Y'"
                    class="is-txt-error">[필수] </span>
                  {{ SVY_ITEM_DATA.SRVY_QITEM_CN }}
                ...
                <div v-if="lastMsg" class="d-flex flex-grow-1 flex-column align-center justify-center flex-wrap is-mt-l">
          ...
          <template v-if="SVY_STEP === 'MULT'">
            ...
            <div class="pl-form-inline-wrap vertical type-2 is-mt-s">
              <div class="pl-form-inline">
                ...
                <!-- <div class="pl-desc is-mt-s" v-for="(chk01, index) in chkTypes" :key="index">
                  <input type="checkbox"
                    v-model="chk01.selected"
                  /><label>{{ chk01.text }}</label>
                </div> -->
                <div v-if="lastMsg" class="d-flex flex-grow-1 flex-column align-center justify-center flex-wrap is-mt-l">
                  <strong class="text-h6 font-weight-bold is-mt-l text-center">수고하셨습니다</strong>
                  <p class="is-mt-s text-center">아래의 제출하기 버튼을 터치하십시오.</p>
            ...
            <div class="pl-form-inline-wrap vertical type-2 is-mt-s">
              <div class="pl-form-inline">
                ...
                <div class="pl-desc is-mt-s">
                  ...
                  <div style="width: 100%" v-show="SVY_ITEM_DATA.HR_APLCN_YN==='Y'">
                    <compo-date-picker
                      DateType="time"
                      StyleProp="width: 100%"
                      :TimesProp.sync='timeStep'
      ...
      <div
        v-if="!svy_done"
        class="pl-btn-wrap justify-center">
        <v-btn class="pl-btn is-sub" @click="prevStepSvy">이전</v-btn>

        <v-btn
          v-if="lastMsg"
          class="pl-btn"
          @click="nextStepSvy2">제출하기</v-btn>
        <v-btn
          v-if="SVY_STEP === SVY_LIST[1] && !lastMsg"
          class="pl-btn"
          @click="nextStepSvy('ok')">확인</v-btn>
        <v-btn
          v-else-if="SVY_STEP !== SVY_LIST[SVY_LIST.length - 1] && !lastMsg"
          class="pl-btn"
          @click="nextStepSvy('next')">다음</v-btn>
      </div>
...
<script>
  ...
  export default {
    name: "MENUSVYPREVIEW", //name은 'MENU_' + 파일명 조합
    props:{
      previewCall: Object
    },
    components: {
    ...
    data() {
      return {
        ...
        SVY_STEP: 'start',
        // SVY_LIST: ['start', 'terms', 'chk', 'm_chk', 'list', 'text', 'textarea', 'date'],
        SVY_LIST: ['start', 'terms', 'SNGL', 'MULT', 'SHOT', 'DESC', 'DATE', ''],
        svyIndex: 0,
        svy_done: false,
    ...
    methods: {
      ...
      async selectMain(){
        ...
        if (resData.HEADER.ERROR_FLAG)
        {
          ...
          if(resData.DATA.length > 0){
            this.O_DATA = resData.DATA;
            this.O_DATA.forEach(e => {
              if(e.HEAD_YN === 'Y'){
                ...
                // if(!this.mixin_isEmpty(e.STRG_FILE_NM)){
                //   let imgUrl = '';
                //   const filePath = e.FILE_PATH.split('/');
                //   filePath.forEach(e => {
                //     imgUrl += `${e}/`
                //   });
                //   imgUrl += e.STRG_FILE_NM;
                //   this.SVY_HEAD_DATA.IMG_URL = imgUrl;
                // }
              }
            });
            //항목id 중복제거
            this.D_DATA = this.O_DATA.filter((item1, idx1)=>{
                return this.O_DATA.findIndex((item2, idx2)=>{
                    if(item1.HEAD_YN === 'N'){
      ...
      //입력한 내용 설정
      setSrvyItemTemp(index){
        if(this.SVY_TEMP_DATA.length > 0){
          const obj = this.SVY_TEMP_DATA[index];
          if(!this.mixin_isEmpty(obj)){
            ...
            if(step === 'SNGL'){
              ...
              this.chkTypes.forEach(e => { e.selected = false; });
              obj.o.forEach(e => {
                for(let i=0; i<this.chkTypes.length; i++){
                  if(e.val === this.chkTypes[i].value){
                    this.chkTypes[i].selected = true;
                    break;
                  }
                }
              });
            }else if(step === 'SHOT'){
              this.CHC_SHOT_DESC  = val;
            }else if(step === 'DESC'){
              this.CHC_DESC  = val;
            }else if(step === 'DATE'){
              this.date = val;
              if(!this.mixin_isEmpty(obj.timeStep)){
      ...
      getSrvyItemTemp(){
        if(this.svyIndex > 0){
          //다음 누르기전 값
          const data = this.D_DATA[this.svyIndex -1];
          const step = data.QITEM_TYPE_CD;
          let o = [];
          //값이 있으면 업데이트
          if(!this.mixin_isEmpty(this.SVY_TEMP_DATA[this.svyIndex -1])){
            if(step === 'SNGL'){
              this.SVY_TEMP_DATA[this.svyIndex -1].val = this.radGrp;
            }else if(step === 'MULT'){
              this.SVY_TEMP_DATA[this.svyIndex -1].o = [];
              this.chkTypes.forEach(e => {
                if(e.selected){
                  this.SVY_TEMP_DATA[this.svyIndex -1].o.push({ val : e.value});
                }
              });
            }else if(step === 'SHOT'){
              this.SVY_TEMP_DATA[this.svyIndex -1].val = this.CHC_SHOT_DESC;
            }else if(step === 'DESC'){
              this.SVY_TEMP_DATA[this.svyIndex -1].val = this.CHC_DESC;
            }else if(step === 'DATE'){
              this.SVY_TEMP_DATA[this.svyIndex -1].val = this.date;
              if(data.HR_APLCN_YN === 'Y'){
                this.SVY_TEMP_DATA[this.svyIndex -1].timeStep = this.timeStep;
              }
            }
          //없으면 인서트
          }else{
            if(step === 'SNGL'){
              this.SVY_TEMP_DATA.push({ step : step, val : this.radGrp });
            }else if(step === 'MULT'){
              this.chkTypes.forEach(e => {
                if(e.selected){
                  o.push({ val : e.value});
              ...
              this.SVY_TEMP_DATA.push({ step : step, o });
            }else if(step === 'SHOT'){
              this.SVY_TEMP_DATA.push({ step : step, val : this.CHC_SHOT_DESC });
            }else if(step === 'DESC'){
              this.SVY_TEMP_DATA.push({ step : step, val : this.CHC_DESC });
            }else if(step === 'DATE'){
              this.SVY_TEMP_DATA.push({ step : step, val : this.date, timeStep : (data.HR_APLCN_YN === 'Y' ? this.timeStep : '') });
            }
          }
        }
      },
      getSrvyItems(){
        this.SVY_ITEM_DATA = this.D_DATA[this.svyIndex -1];
        if(this.mixin_isEmpty(this.SVY_ITEM_DATA)){
        ...
        this.SVY_STEP = this.SVY_ITEM_DATA.QITEM_TYPE_CD;
        this.requiredFlag = (this.SVY_ITEM_DATA.ESNTL_YN === 'Y') ? true : false;
        let i = 0;
        this.radGrp = '';
        this.radioGroup = [];
        this.chkTypes = [];
        ...
        this.O_DATA.forEach(e => {
          if(e.HEAD_YN === 'N'){
            if(this.SVY_ITEM_DATA.SRVY_QITEM_ID === e.CHC_SRVY_QITEM_ID){

              if(this.SVY_STEP === 'SNGL'){
                this.radioGroup.push({
                  text : this.mixin_decode(e.QITEM_CHC_CN), value : e.QITEM_CHC_ID
                })
                // this.radioGroup[i] = {
                //   text : e.QITEM_CHC_CN, value : e.QITEM_CHC_ID
                // }
              }else if(this.SVY_STEP === 'MULT'){
                this.chkTypes.push({
                  text : this.mixin_decode(e.QITEM_CHC_CN), selected : false, value : e.QITEM_CHC_ID
                })
                // this.chkTypes[i] = {
                //   text : e.QITEM_CHC_CN, selected : false, value : e.QITEM_CHC_ID
                // }
              }
              i++;
            }else{
        ...
        if(val === 'start'){
          ...
          if(this.SVY_HEAD_DATA.CLCT_AGRE_USE_YN === 'Y'){
            this.SVY_STEP = 'terms';
             ...
             //마지막이면
             if(this.D_DATA.length -1 === this.svyIndex){
              this.lastMsg = true;
            }
            this.getSrvyItemTemp();
            this.svyIndex++;
            this.getSrvyItems();
            this.setSrvyItemTemp(this.svyIndex-1);
          }
        }else{
          if(val === 'ok'){
            let eFlag = true;
            ...
            //필수체크
            if(!this.chkItemValid()) return false;
      ...
      //항목저장
      async setItems(f){
        let tp = this.SVY_ITEM_DATA.QITEM_TYPE_CD;
        const sUrl = '/api/svy/preview/insertitem';
        const headParam = {
          head: {
            'SERVICE' : 'svy.preview.insertitem',
            'METHOD' : 'insertitem',
            'TYPE' : 'BIZ_SERVICE',
          }
        }
        ...
        let oData = {};
        let tempSrvyQitemId = this.SVY_ITEM_DATA.SRVY_QITEM_ID;
        if(tp === 'SNGL'){    // 단일
          if(this.mixin_isEmpty(this.radGrp)) tempSrvyQitemId = -1; //return;
          oData = {
            SRVY_TRGT_ID : this.TRGT_ID,
            SRVY_QITEM_ID : tempSrvyQitemId,
            QITEM_CHC_ID : this.radGrp,
            CHC_YN : 'Y',
            FINISH : f
          }
        }else if(tp === 'MULT'){  // 멀티
          ...
          if(tp === 'SHOT' && this.mixin_isEmpty(this.CHC_SHOT_DESC)) tempSrvyQitemId = -1;//return;
          if(tp === 'DESC' && this.mixin_isEmpty(this.CHC_DESC)) tempSrvyQitemId = -1;//return;

          let val = '';
          if(tp === 'SHOT'){
            val = this.CHC_SHOT_DESC;
          }else if(tp === 'DESC'){
            val = this.CHC_DESC;
          }else if(tp === 'DATE'){
            val = this.date + (this.SVY_ITEM_DATA.HR_APLCN_YN === 'Y' ? ` ${this.timeStep}` : '')
          }
        ...
        let resData = await this.common_postCall(sUrl, (tp === 'MULT' ? postParam.sendData : oData), headParam );
        if (resData.HEADER.ERROR_FLAG)
        {
          this.errChk(resData);
        }
