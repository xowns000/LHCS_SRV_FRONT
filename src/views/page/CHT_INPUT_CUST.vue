...
export default {
  ...
  data: () => ({
    ...
    MESSAGE : {
      ...
      ALERT : {
        INPUT_IMPSBLTY : {alertDialogToggle: true, msg: '지금은 고객님의 정보를 입력하실 수 없습니다', iconClass: 'is-info', type: 'default'}
      },
      INFO : {
  ...
  methods: {
    ...
    //암호화 키 호출
    async getKey() {
      //멀티테넌시(multitenancy) 처리 : 로그인 전 로그인 ID로 테넌트 정보 조회
    ...
    async setCustInfo(){
      console.log("this.$route.query.user_key", this.$route.query.user_key);
      console.log("this.$route.query.sender_key", this.$route.query.sender_key);
      let params = {
        method: "post",
        url: process.env.VUE_APP_API_URL + "message",
        headers: {},
        data : {
          user_key: this.$route.query.user_key
          , sender_key: this.$route.query.sender_key
          , serial_number:"1689884130894833200"
          , time: 1522387086667
          , type : "custInfo"
          , content:"{CUST_NM : "+ this.CUST_NM +", PHONE_NO : " + this.PHONE_NO + " }"
          , session_id : "123"
    ...
    //현재 고객이 상담중인지 체크
    //상담중이지 않으면 고객정보를 입력받을 수 없다
    //고객정보를 입력받아도 고객정보를 확인할 상담원이 없기때문
    async chkInputPsblty(){
      let sUrl = '/chat-api/main/chkInputPsblty';
      let postParam = {
        CHT_USER_KEY: this.$route.query.user_key
        , DSPTCH_PRF_KEY: this.$route.query.sender_key
      }
      let headParam = {
        head: {
        }
      ...
      if(resData.error != "-999"){
        ...
        if(resData.DATA.length > 0){
          this.INPUT_BTN = false;
        } else {
          this.INPUT_BTN = true;
          this.showAlert(this.MESSAGE.ALERT.INPUT_IMPSBLTY);
        }
      } else {
