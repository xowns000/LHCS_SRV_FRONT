  ...
  <div class="login-wrap">
    <div class="login-main">
      <div class="form-inp sm">
        <!-- POST API 표준인증창 불러오기 -->
        <form name="CXWindow" method="post" style="display:none">
          <input type="hidden" ref="url" name="url" value="" />
          <input type="hidden" ref="certInfo" name="certInfo" value="" />
          <input type="hidden" ref="callbackUrl" name="callbackUrl" value="" />
          <input type="hidden" ref="authToken" name="authToken" value="" />
        </form>
        <!-- POST API 표준인증창 불러오기 -->

        <!--  이용기관 포탈 "간편인증" 샘플 버튼
          TODO css class 명 하나로 분리 하여 oacx-ux.css가 아닌 독립적인 CSS분리 필요  -->
        <div class=oacx-demo-auth>
          <!-- <button id=btn1 type=button class="oacx-ux oacx-ux-btn btn btn-primary demo-btn"> 
          간편인증 <span>현재 창(앱)</span>
          </button>
          <br><br>
          <button id=btn2 type=button class="oacx-ux oacx-ux-btn btn btn-primary demo-btn"> 
          간편인증 <span>새 창(PC)</span>
          </button> -->
...
export default {
  ...
  data: () => ({
    ...
    PASSWORD_NEW: "",
    PASSWORD_NEW2: "",
    POP_URL:'https://ida.omnione.net:7443/sign/',
    CUST_NM:"",
    PHONE_NO:"",
  }),
  ...
  methods: {
    ...
    setCust(){
      this.$api
      .post(
        "/api/palette-main/setOmniCust",
        {
          CUST_NM: this.CUST_NM,
          PHONE_NO: this.PHONE_NO.replace(/\-/g, ""),
        },
        {
          head: {
            SERVICE: "palette-main",
            METHOD: "setOmniCust",
            TYPE: "BIZ_SERVICE",
            PAGES_CNT: 1,
            ROW_CNT: 50,
    ...
    setOmni(){
      this.$api
      .post(
        "/api/palette-main/getOmnioneKey",
        {
        },
        {
          head: {
            SERVICE: "palette-main",
            METHOD: "getOmnioneKey",
            TYPE: "BIZ_SERVICE",
            PAGES_CNT: 1,
            ROW_CNT: 50,
          },
      ...
      .then((response) => {
        ...
        if(response.data.DATA[0].resultCode == "200"){
          let certInfo = response.data.DATA[0].certInfo;
          let callbackUrl = process.env.VUE_APP_API_URL + response.data.DATA[0].callbackUrl;
          let authToken = response.data.DATA[0].authToken;

          this.$refs.url.value = this.POP_URL;
          this.$refs.certInfo.value = certInfo;
          this.$refs.callbackUrl.value = callbackUrl;
          this.$refs.authToken.value = authToken;

          window.open("", "CXWindow");
          var dw = document.CXWindow;
          dw.action = this.POP_URL;
          dw.target = "CXWindow";
          dw.submit();
        }
      })
